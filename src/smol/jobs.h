...
namespace smol::jobs
{
    ...
    SMOL_API void kick(const std::function<void()>& task, counter_t* counter = nullptr,
                       priority_e prio = priority_e::HIGH);
    SMOL_API void dispatch(u32_t count, u32_t batch_size, const std::function<void(u32_t, u32_t)>& task,
