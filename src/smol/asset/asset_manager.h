...
namespace smol::asset_manager
{
    ...
    struct loader_entry_t {
        ...
        loader_entry_t(
            ...
            std::type_index arg_type_index
        )
        : loader(std::move(loader_func)), arg_type(arg_type_index) {}
    };
    ...
    void register_loader()
    {
        ...
        get_asset_loaders().insert_or_assign(typeid(T), loader_entry_t {
            [](const std::string& path, const std::any& _) -> std::shared_ptr<smol::asset::asset_t> {
    ...
    void register_loader_with_args()
    {
        ...
        get_asset_loaders().insert_or_assign(typeid(T), loader_entry_t {
            [](const std::string& path, const std::any& a) -> std::shared_ptr<smol::asset::asset_t> {
    ...
    asset_handle_t load_sync(const std::string& path, const Args& args)
    {
        ...
        {
            SMOL_LOG_ERROR("ASSET_MANAGER", "Loader for '{}' expects argument type '{}', but got '{}'",
            path, loader_it->second.arg_type.name(), typeid(Args).name());
            return asset_handle_t { "" };
        }
