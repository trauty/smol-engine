...
namespace smol::components
{
    ...
    camera_ct::camera_ct(f32 fov_deg, f32 near_plane, f32 far_plane, f32 aspect)
    ...
    void camera_ct::start()
    {
        ...
        sub_id = smol::events::subscribe<smol::window::size_changed_event_t>(
            [this](const smol::window::size_changed_event_t& ctx) { aspect = (f32)ctx.width / (f32)ctx.height; });
    }
