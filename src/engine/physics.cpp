...
namespace smol::physics
{
    namespace
    {
        ...
        public:
            ...
            u32_t GetNumBroadPhaseLayers() const override { return broad_phase_layers_e::COUNT; }

            JPH::BroadPhaseLayer GetBroadPhaseLayer(JPH::ObjectLayer layer) const override
            {
                broad_phase_layer_iter_t it = object_to_broadphase_layer.find(layer);
                if (it != object_to_broadphase_layer.end()) return JPH::BroadPhaseLayer(it->second);
                
                return JPH::BroadPhaseLayer(broad_phase_layers_e::DYNAMIC);
            }
#if defined(JPH_EXTERNAL_PROFILE) || defined(JPH_PROFILE_ENABLED) // falls wir Profiling anmachen in Jolt
